...
dist1T<-dist2T<-dist1R<-dist2R<-matrix(NA,pn,sn)
pdbios<-cmud<-list(NA)
...
for(t in 1:Tmax){
  ...
  pd<-distpT-distpR
  pdbios<-lapply(1:sn, function(r) which(abs(pd[,r])<0.15))
  ...
  clusterExport(cl, c('pn','sn','x','addsample',
                      ...
                      'xbar1T','sdT','xbar2T','xbar1R','sdR','xbar2R'))
  ...
  mu_bios<-sapply(1:sn, function(r) length(which(abs(cmud[[r]]) < 0.223))/length(pdbios[[r]]))
